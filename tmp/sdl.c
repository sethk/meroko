...
#ifdef MOUSE_WARPING
static int mouse_warping = 0;

static void sdl_warp_explorer_mouse(void)
{
  int x, y;
...
#if 1
  if (1) logmsgf("Warp Explorer mouse from (%d, %d) to (%d,%d)\n",
		 sib_mouse_x_pos, sib_mouse_y_pos, x, y);
  sib_mouse_x_pos = x;
  sib_mouse_y_pos = y;
#else
  mouse_warping = 1;
  if (1) logmsgf("Warping X11 mouse from (%d, %d) to (%d, %d)\n",
		 x, y, sib_mouse_x_pos, sib_mouse_y_pos);
  SDL_WarpMouse(sib_mouse_x_pos, sib_mouse_y_pos);
#endif
...
#ifdef MOUSE_WARPING
  if (mouse_warping) {
    if (1) logmsg("Not moving mouse (being warped)\n");
    mouse_warping = 0;
    return;
  }
#endif
	...
	sib_mouse_x_pos = (sib_mouse_x_pos < 0 ? 0 :
			   sib_mouse_x_pos > video_width ? video_width : sib_mouse_x_pos);
	sib_mouse_y_pos = (sib_mouse_y_pos < 0 ? 0 :
			   sib_mouse_y_pos > video_height ? video_height : sib_mouse_y_pos);
...
sdl_video_write(int offset, unsigned int bits)
{
	if (screen) {
		...
		if (offset > MAX_BITMAP_OFFSET) {
#if 0
		  logmsgf("sdl_video_write: offset too big (0x%x > 0x%x): 0x%x\n",
...
#if 0
/* The sib_mouse_{x,y}_pos doesn't really match the Explorer screen position.
   Need to use deltas instead.
*/
		  if ((ev->active.gain) &&
		      (ev->active.state == SDL_APPMOUSEFOCUS)) {
		    sdl_warp_explorer_mouse();
		  }
#endif
			break;
		default:
...
{
#if 0
  /* Ignore size given - in particular width must be 1024 */
  if ((width > VIDEO_WIDTH) || (height > VIDEO_HEIGHT)) {
    logmsgf("**** Too large screen requested: %d x %d (max is %d x %d)\n",
