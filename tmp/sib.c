...
//  map['+'] = 0x2f; // + is shifted =
  ...
  map[SDLK_F7] = 015;           /* F7 - Clear Screen */
...
#endif
#if 1 // BV
 map[SDLK_RCTRL] = 0x1a;        /* Make right Ctrl a Super key */
#else
...
void sib_nubus_io(){
  ...
  /*
  if(sib_log_arm > 0){
    logmsgf("SIB: Log: Op %ld for address %lX\n",NUbus_Request,NUbus_Addr);
    sib_log_arm--;
  }
  */
...
//      logmsgf("SIB: sec RD: %X\n",sib_rtc_sec);
    ...
    case 0xFC0004: // Keyboard USART Recieve Data
      ...
      //      if((keyboard_usart_status&0x02) != 0 && (keyboard_usart_status&0x78)==0){ // && keyboard_usart_rx_bot != keyboard_usart_rx_top){
        // Return data
      if (keyboard_usart_rx_bot != keyboard_usart_rx_top)
        {
